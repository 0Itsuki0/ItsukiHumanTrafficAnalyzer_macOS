...
class VideoManager {
    ...
    func loadVideo(_ url: URL) async throws {
        ...
        if let videoTrack = tracks?.first {
            ...
            do {
                ...
                throw TrackingModel.TrackingError.loadingFailed(message: "error loading data: \(error.localizedDescription).")
            }

        } else {
            print("error loading tracks.")
            throw TrackingModel.TrackingError.loadingFailed(message: "error loading track.")
        }
    ...
    private func readAsset() throws {
        guard self.videoAsset != nil, self.videoTrack != nil else {
            ...
            throw TrackingModel.TrackingError.loadingFailed(message: "nil video reader output")
        }
        ...
        do {
            ...
            throw TrackingModel.TrackingError.loadingFailed(message: "Failed to create AVAssetReader object: \(error.localizedDescription).")
        }
        ...
        guard self.assetReader!.canAdd(videoAssetReaderOutput!) else {
            ...
            throw TrackingModel.TrackingError.loadingFailed(message: "cannot add output.")
        }
        
        self.assetReader!.add(videoAssetReaderOutput!)
        guard self.assetReader!.startReading() else {
            print("Fail to start reading video.")
            throw TrackingModel.TrackingError.loadingFailed(message: "Fail to start reading video.")
        }
    }
